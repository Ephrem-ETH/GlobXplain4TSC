...
def run_single_iteration(run, X, y, model_name, class_names, base_dir):
    ...
    try:
        ...
        return train_acc, valid_acc, test_acc, dt_acc, fidelity, depth, n_nodes, robustness_score
...
def monte_carlo_cross_val_parallel(num_runs, model_name, class_names, base_dir):
    ...
    train_accs, valid_accs, test_accs, dt_accs, fidelities, depths, nodes, robust_score = zip(*results)
    ...
    np.savetxt(f'{base_dir}/results.txt', np.array([train_accs, valid_accs, test_accs, dt_accs, fidelities, depths, nodes, robust_score]).T, header=header)
    ...
    dt_mean, dt_std = round(np.mean(dt_accs), 2), round(np.std(dt_accs), 2)
    fidelity_mean, fidelity_std = round(np.mean(fidelities), 2),  round(np.std(fidelities), 2)
    depth_mean, depth_std = round(np.mean(depths)), round(np.std(depths))
    nodes_mean, nodes_std = round(np.mean(nodes)), round(np.std(nodes))
    robust_mean, robust_std = round(np.mean(robust_score), 2),  round(np.std(robust_score), 2)
...
if __name__ == "__main__":
    ...
    try:
        ...
        forda_fcn_result = [
            ...
            ["DT Accuracy", f"{result_dict['dt_acc_mean']:.2f} \u00B1 {result_dict['dt_acc_std']:.2f}"],
            ["Fidelity", f"{result_dict['fidelity_mean']:.2f} \u00B1 {result_dict['fidelity_std']:.2f}"],
            ["Depth", f"{result_dict['depth_mean']} \u00B1 {result_dict['depth_std']}"],
            ["#Nodes", f"{result_dict['nodes_mean']} \u00B1 {result_dict['nodes_std']}"],
            ["Robustness", f"{result_dict['robust_mean']:.2f} \u00B1 {result_dict['robust_std']:.2f}"]
        ]

        print(f"Info:\nDataset: {args.dataset}\nModel: {args.model}\nNum runs: {args.num_runs}")
        table_headers = ["Metric", f" Mean \u00B1 Std"]
        print(tabulate(forda_fcn_result, headers=table_headers, tablefmt="grid"))
